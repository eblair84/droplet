...
class Sec_Xml_Template:

	def __init__(self):
		x = 5

	def what_quarter(self, date):
		theDay = date # date.today()
		today = theDay - timedelta(days = 1)
		month = today.month
		return (month-1)//3+1

	def create_url(self, base, parts):
		url = base
	...
	def parse_to_components(self, url):
		print ("parse to components")
		return 'parse to components'

	def latest_filing(self, ticker):
		url = 'https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=' + ticker + '&type=&dateb=&owner=include&start=0&count=40&output=atom'
		return "latest filing"

	def pre_company(self, ticker):
		url = 'https://www.sec.gov/cgi-bin/browse-edgar?action=getcompany&CIK=' + ticker + '&type=&dateb=&owner=include&start=0&count=40&output=atom'
		r = requests.get(url)
		soup = BeautifulSoup(r.text,'lxml')
		nameDict = {}
		oldNames = soup.find_all('names')
		for index, name in enumerate(oldNames[0:]):
# print('Name: [{}] Date: [{}]'.format(name.find('name').text, name.find('date').text))
			nameDict[name.find('date').text] = name.find('name').text
		return nameDict

	def function_to_perform(self, function,operand):
		switch = {
			'filing': lambda: parse_to_components(operand),
			'previous': lambda: pre_company(operand),
			'latest': lambda: latest_filing(operand)
		}
		return switch.get(function, 'Invalid function')
...
#dataUrl = 'https://www.sec.gov/cgi-bin/browse-edgar?company=pareteum&owner=include&action=getcompany'
	...
	singleCoUrl = 'https://www.sec.gov/cgi-bin/browse-edgar?company=pareteum&owner=include&action=getcompany&count=100'
# dataUrl = 'https://www.sec.gov/Archives/edgar/daily-index/' + str(today.year) + '/QTR' + str(qtr) + '/'
	filingUrl = 'https://www.sec.gov/Archives/'
# r = requests.get(singleCoUrl)
# 	function = sys.argv[1]
# 	ticker = sys.argv[2]
# function_to_perform(function, ticker)
